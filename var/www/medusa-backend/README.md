...
| `MEDUSA_DB_SSL` / `MEDUSA_ADMIN_DB_SSL` | Force the bootstrap scripts to enable (`require/true`) or disable (`disable/false`) SSL when connecting to Postgres. When unset the scripts inspect `DATABASE_URL` and default to non-SSL for the Docker stack. |
