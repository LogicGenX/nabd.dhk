    ...
    <ol className='flex items-center justify-center mb-6 text-sm'>
      {steps.map((step, idx) => (
        <li key={step} className='flex items-center'>
          <span className={`px-2 ${idx === current ? 'font-bold' : 'text-gray-600'}`}>{step}</span>
          {idx < steps.length - 1 && <span className='mx-1 text-gray-400'>&rarr;</span>}
        </li>
      ))}
    </ol>
  )
    ...
    <main className='max-w-md mx-auto p-4 sm:p-8 space-y-6'>
      ...
      <form className='space-y-6' onSubmit={handleSubmit} noValidate>
        <section className='space-y-4'>
          ...
          <div>
            <label htmlFor='checkout-name' className='block mb-1'>Name</label>
            <input
              id='checkout-name'
              name='name'
              type='text'
          ...
          <div>
            <label htmlFor='checkout-address' className='block mb-1'>Address</label>
            <input
              id='checkout-address'
              name='address'
              type='text'
        ...
        <section className='space-y-4'>
          ...
          <div className='p-3 border border-gray-700 rounded-md'>Standard shipping (3-5 days)</div>
        </section>

        <section className='space-y-4'>
          <h2 className='text-xl font-semibold'>Payment method</h2>
          <div>
            <label htmlFor='checkout-payment' className='block mb-1'>Select method</label>
            <select
              id='checkout-payment'
              name='payment'
              value={form.payment}
        ...
        <button type='submit' className='w-full bg-accent text-white py-3 rounded-md hover:bg-accent/90'>
          Place order
        </button>
